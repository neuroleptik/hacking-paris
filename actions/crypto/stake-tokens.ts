...
export const stakeTokens = authActionClient
  ...
  .action(async ({ parsedInput, ctx: { session } }: {
    ...
    try {
      ...
      const recoveredAddress = ethers.verifyMessage(parsedInput.message, parsedInput.signature);
      if (recoveredAddress.toLowerCase() !== user.walletAddress.toLowerCase()) {
        throw new Error('Signature invalide');
      }

      // Vérifier le solde de l'utilisateur
      const tokenContract = new ethers.Contract(parsedInput.tokenAddress, erc20Abi, provider);
      ...
      const allowedTokens = await stakingContract.getAllowedTokensWithTotalStaked();
      const isTokenAllowed = allowedTokens[0].includes(parsedInput.tokenAddress);
      ...
      if (!privateKey) {
        console.log('⚠️  MODE TEST: Aucune vraie transaction blockchain - pas de frais de gas');
        console.log('Pour une vraie transaction avec frais, définissez PRIVATE_KEY dans vos variables d\'environnement');
      ...
      console.log('🚀 MODE PRODUCTION: Transaction blockchain avec frais de gas');
      ...
      console.log('Prix du gas:', ethers.formatUnits(gasPrice.gasPrice || 0, 'gwei'), 'gwei');

      // Créer les instances de contrats avec le wallet
      const tokenContractWithWallet = new ethers.Contract(parsedInput.tokenAddress, erc20Abi, wallet);
      const stakingContractWithWallet = new ethers.Contract(contractAddress, stakingContractAbi, wallet);

      // Approuver le contrat de staking pour dépenser les tokens
      console.log('📝 Transaction 1/2: Approbation des tokens...');
      const approveTx = await tokenContractWithWallet.approve(contractAddress, amountToStake);
      ...
      const stakeTx = await stakingContractWithWallet.stake(parsedInput.tokenAddress, amountToStake);
      ...
      throw new Error(`Impossible de staker les tokens: ${error instanceof Error ? error.message : 'Erreur inconnue'}`);
    }
  });
