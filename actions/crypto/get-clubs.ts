import { Chiliz, Network, TatumSDK } from '@tatumio/tatum';
...
async function getTokenInfo(
  ...
  try {
    ...
    console.error(
      `Erreur lors de la récupération des infos du token ${tokenAddress}:`,
      error
    );
    ...
    async ({ ctx: { session } }: { ctx: { session: Session | null } }) => {
      ...
      try {
        ...
        try {
          ...
          const tokenInfoPromises = addresses.map(
            async (address: string, i: number): Promise<ClubInfo> => {
          ...
          console.error(
            "Erreur lors de l'appel de getAllowedTokensWithTotalStaked:",
            functionError
          );
        ...
        throw new Error(
          `Impossible de récupérer les clubs: ${error instanceof Error ? error.message : 'Erreur inconnue'}`
        );
      }
    }
  );
